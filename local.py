...
collection = mongo_db["durr"]
mongo_collection = mongo_db["behr"]
...
for cls in class_names:
    ...
    if existing_document:
        ...
        collection.insert_one(
            {"name": cls, "iniImg": f"data:image/jpeg;base64,{encoded_string}", "latest timing": datetime.now().strftime('%H:%M:%S'), "date": datetime.now().strftime('%Y-%m-%d'), "camID": camidno }
        )
